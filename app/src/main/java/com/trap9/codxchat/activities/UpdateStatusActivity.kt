...
class UpdateStatusActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.extras != null) {
            ...
            edt_status_profile.setText(status)
        }

        btn_update_status.setOnClickListener {
            var updateStatus = edt_status_profile.text.toString().trim()
            ...
            var statusRef = mDatabase!!.reference.child("Users").child(uid).child("status")
                statusRef.setValue(updateStatus).addOnCompleteListener { task: Task<Void> ->
